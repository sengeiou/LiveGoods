  ...
  <div>
    <div class="buy-store-container clear-fix">
      ...
      <div class="item-container float-right" @click="clickBuyHandler">
        <button :disabled='disableclick' @click="buy" :class="{'jin':disableclick}">{{buyaction}}<i v-if='buytime'>({{buytime}})</i></button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
      ...
      .then(data=>{
	    ...
	    let left = (data.data.time - now)/1000
	    if(left>0){
